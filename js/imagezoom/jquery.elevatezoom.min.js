...
(function (d, c, a, e) {
  var b = {
    init: function (g, h) {
      ...
      f.elem = h;
      f.$elem = d(h);
      f.imageSrc = f.$elem.data("zoom-image")
        ? f.$elem.data("zoom-image")
        : f.$elem.attr("src");
      f.options = d.extend({}, d.fn.elevateZoom.options, g);
      if (f.options.tint) {
      ...
      f.refresh(1);
      d("#" + f.options.gallery + " a").click(function (i) {
        if (f.options.galleryActiveClass) {
          d("#" + f.options.gallery + " a").removeClass(
            f.options.galleryActiveClass
          );
          d(this).addClass(f.options.galleryActiveClass);
        }
        i.preventDefault();
        if (d(this).data("zoom-image")) {
          f.zoomImagePre = d(this).data("zoom-image");
        } else {
          f.zoomImagePre = d(this).data("image");
        }
        f.swaptheimage(d(this).data("image"), f.zoomImagePre);
        return false;
      });
    },
    refresh: function (g) {
          ...
          .bind("mousewheel DOMMouseScroll MozMousePixelScroll", function (h) {
            ...
            if (i / 120 > 0) {
              ...
              if (g.options.maxZoomLevel) {
                ...
                g.changeZoomLevel(
                  parseFloat(g.currentZoomLevel) + g.options.scrollZoomIncrement
                );
              }
            }
            return false;
    ...
    setPosition: function (g) {
      ...
      if (f.options.tint && f.options.zoomType != "inner") {
        f.zoomTint.css({ top: 0 });
        f.zoomTint.css({ left: 0 });
      }
      if (f.options.responsive && !f.options.scrollZoom) {
        if (f.options.showLens) {
          ...
          if (f.largeWidth < f.options.zoomWindowWidth) {
            lensWidth = f.nzWidth;
          } else {
          ...
          if (f.options.zoomType != "lens") {
            ...
            if (f.nzWidth < f.options.zoomWindowHeight / f.heightRatio) {
              lensWidth = f.nzWidth;
            } else {
              lensWidth = String(f.options.zoomWindowWidth / f.widthRatio);
            }
      ...
      f.zoomContainer.css({ top: f.nzOffset.top });
      f.zoomContainer.css({ left: f.nzOffset.left });
      f.mouseLeft = parseInt(g.pageX - f.nzOffset.left);
      f.mouseTop = parseInt(g.pageY - f.nzOffset.top);
      if (f.options.zoomType == "window") {
        f.Etoppos = f.mouseTop < f.zoomLens.height() / 2;
        f.Eboppos =
          f.mouseTop >
          f.nzHeight - f.zoomLens.height() / 2 - f.options.lensBorderSize * 2;
        f.Eloppos = f.mouseLeft < 0 + f.zoomLens.width() / 2;
        f.Eroppos =
          f.mouseLeft >
          f.nzWidth - f.zoomLens.width() / 2 - f.options.lensBorderSize * 2;
      }
      if (f.options.zoomType == "inner") {
        ...
        f.Eroppos =
          f.mouseLeft >
          f.nzWidth -
            f.nzWidth / 2 / f.widthRatio -
            f.options.lensBorderSize * 2;
      }
      if (
        f.mouseLeft < 0 ||
        f.mouseTop < 0 ||
        f.mouseLeft > f.nzWidth ||
        f.mouseTop > f.nzHeight
      ) {
        f.setElements("hide");
        return;
      } else {
        if (f.options.showLens) {
          f.lensLeftPos = String(
            Math.floor(f.mouseLeft - f.zoomLens.width() / 2)
          );
          f.lensTopPos = String(
            Math.floor(f.mouseTop - f.zoomLens.height() / 2)
          );
        }
        if (f.Etoppos) {
          f.lensTopPos = 0;
        ...
        if (f.options.zoomType == "window") {
          if (f.Eboppos) {
            f.lensTopPos = Math.max(
              f.nzHeight - f.zoomLens.height() - f.options.lensBorderSize * 2,
              0
            );
          }
          if (f.Eroppos) {
            f.lensLeftPos =
              f.nzWidth - f.zoomLens.width() - f.options.lensBorderSize * 2;
          }
        }
        if (f.options.zoomType == "inner") {
          if (f.Eboppos) {
            f.lensTopPos = Math.max(
              f.nzHeight - f.options.lensBorderSize * 2,
              0
            );
          }
          if (f.Eroppos) {
            f.lensLeftPos =
              f.nzWidth - f.nzWidth - f.options.lensBorderSize * 2;
          }
        }
        if (f.options.zoomType == "lens") {
          f.windowLeftPos = String(
            ((g.pageX - f.nzOffset.left) * f.widthRatio -
              f.zoomLens.width() / 2) *
              -1
          );
          f.windowTopPos = String(
            ((g.pageY - f.nzOffset.top) * f.heightRatio -
              f.zoomLens.height() / 2) *
              -1
          );
          f.zoomLens.css({
          ...
          if (f.changeBgSize) {
            if (f.nzHeight > f.nzWidth) {
              if (f.options.zoomType == "lens") {
                f.zoomLens.css({
                  "background-size":
                    f.largeWidth / f.newvalueheight +
                    "px " +
                    f.largeHeight / f.newvalueheight +
                    "px",
                });
              }
              f.zoomWindow.css({
              ...
              if (f.options.zoomType == "lens") {
                f.zoomLens.css({
                  "background-size":
                    f.largeWidth / f.newvaluewidth +
                    "px " +
                    f.largeHeight / f.newvaluewidth +
                    "px",
                });
              }
              f.zoomWindow.css({
    ...
    showHideWindow: function (g) {
      ...
      if (g == "show") {
        if (!f.isWindowActive) {
          if (f.options.zoomWindowFadeIn) {
            f.zoomWindow
              .stop(true, true, false)
              .fadeIn(f.options.zoomWindowFadeIn);
          } else {
            f.zoomWindow.show();
          }
      ...
      if (g == "hide") {
        if (f.isWindowActive) {
          if (f.options.zoomWindowFadeOut) {
            f.zoomWindow
              .stop(true, true)
              .fadeOut(f.options.zoomWindowFadeOut, function () {
                if (f.loop) {
      ...
      if (g == "show") {
        if (!f.isLensActive) {
          if (f.options.lensFadeIn) {
            f.zoomLens.stop(true, true, false).fadeIn(f.options.lensFadeIn);
          } else {
            f.zoomLens.show();
          }
          f.isLensActive = true;
      ...
      if (g == "hide") {
        if (f.isLensActive) {
          if (f.options.lensFadeOut) {
            f.zoomLens.stop(true, true).fadeOut(f.options.lensFadeOut);
          } else {
            f.zoomLens.hide();
          }
      ...
      if (g == "show") {
        if (!f.isTintActive) {
          if (f.options.zoomTintFadeIn) {
            f.zoomTint
              .css({ opacity: f.options.tintOpacity })
              .animate()
              .stop(true, true)
              .fadeIn("slow");
          } else {
            f.zoomTint.css({ opacity: f.options.tintOpacity }).animate();
            f.zoomTint.show();
          }
          f.isTintActive = true;
        }
    ...
    setWindowPostition: function (g) {
      var f = this;
      if (!isNaN(f.options.zoomWindowPosition)) {
        switch (f.options.zoomWindowPosition) {
          case 1:
            f.windowOffsetTop = f.options.zoomWindowOffety;
            f.windowOffsetLeft = +f.nzWidth;
            break;
          case 2:
            if (f.options.zoomWindowHeight > f.nzHeight) {
              f.windowOffsetTop =
                (f.options.zoomWindowHeight / 2 - f.nzHeight / 2) * -1;
              f.windowOffsetLeft = f.nzWidth;
            } else {
            }
            break;
          case 3:
            f.windowOffsetTop =
              f.nzHeight - f.zoomWindow.height() - f.options.borderSize * 2;
            f.windowOffsetLeft = f.nzWidth;
            break;
          case 4:
          ...
          case 5:
            f.windowOffsetTop = f.nzHeight;
            f.windowOffsetLeft =
              f.nzWidth - f.zoomWindow.width() - f.options.borderSize * 2;
            break;
          case 6:
            if (f.options.zoomWindowHeight > f.nzHeight) {
              f.windowOffsetTop = f.nzHeight;
              f.windowOffsetLeft =
                (f.options.zoomWindowWidth / 2 -
                  f.nzWidth / 2 +
                  f.options.borderSize * 2) *
                -1;
            } else {
            }
            break;
          ...
          case 8:
            f.windowOffsetTop = f.nzHeight;
            f.windowOffsetLeft =
              (f.zoomWindow.width() + f.options.borderSize * 2) * -1;
            break;
          case 9:
            f.windowOffsetTop =
              f.nzHeight - f.zoomWindow.height() - f.options.borderSize * 2;
          ...
          case 12:
            f.windowOffsetTop =
              (f.zoomWindow.height() + f.options.borderSize * 2) * -1;
            f.windowOffsetLeft =
              (f.zoomWindow.width() + f.options.borderSize * 2) * -1;
            break;
          case 13:
        ...
        f.externalContainer = d("#" + f.options.zoomWindowPosition);
      ...
      f.isWindowSet = true;
      f.windowOffsetTop = f.windowOffsetTop + f.options.zoomWindowOffety;
      f.windowOffsetLeft = f.windowOffsetLeft + f.options.zoomWindowOffetx;
      f.zoomWindow.css({ top: f.windowOffsetTop });
      f.zoomWindow.css({ left: f.windowOffsetLeft });
      if (f.options.zoomType == "inner") {
        f.zoomWindow.css({ top: 0 });
        f.zoomWindow.css({ left: 0 });
      }
      f.windowLeftPos = String(
        ((g.pageX - f.nzOffset.left) * f.widthRatio -
          f.zoomWindow.width() / 2) *
          -1
      );
      f.windowTopPos = String(
        ((g.pageY - f.nzOffset.top) * f.heightRatio -
          f.zoomWindow.height() / 2) *
          -1
      );
      if (f.Etoppos) {
        f.windowTopPos = 0;
      ...
      if (f.Eboppos) {
        f.windowTopPos =
          (f.largeHeight / f.currentZoomLevel - f.zoomWindow.height()) * -1;
      }
      if (f.Eroppos) {
        f.windowLeftPos =
          (f.largeWidth / f.currentZoomLevel - f.zoomWindow.width()) * -1;
      }
      if (f.fullheight) {
        f.windowTopPos = 0;
      }
      ...
      if (f.options.zoomType == "window" || f.options.zoomType == "inner") {
        ...
        if (f.options.zoomType == "window") {
          if (f.largeHeight < f.options.zoomWindowHeight) {
            f.windowTopPos = 0;
          }
          if (f.largeWidth < f.options.zoomWindowWidth) {
            f.windowLeftPos = 0;
          ...
          if (!f.loop) {
            f.loop = setInterval(function () {
              ...
              if (f.scrollingLock) {
                ...
                f.zoomWindow.css({
                  backgroundPosition:
                    f.windowLeftPos + "px " + f.windowTopPos + "px",
                });
                ...
                if (
                  Math.round(
                    Math.abs(f.xp - f.windowLeftPos) +
                      Math.abs(f.yp - f.windowTopPos)
                  ) < 1
                ) {
                  clearInterval(f.loop);
                  f.zoomWindow.css({
    ...
    setTintPosition: function (g) {
      ...
      f.nzOffset = f.$elem.offset();
      f.tintpos = String(
        (g.pageX - f.nzOffset.left - f.zoomLens.width() / 2) * -1
      );
      f.tintposy = String(
        (g.pageY - f.nzOffset.top - f.zoomLens.height() / 2) * -1
      );
      if (f.Etoppos) {
        f.tintposy = 0;
      }
      ...
      if (f.Eroppos) {
        f.tintpos =
          (f.nzWidth - f.zoomLens.width() - f.options.lensBorderSize * 2) * -1;
      }
      if (f.options.tint) {
        if (f.fullheight) {
          f.tintposy = 0;
        ...
        f.zoomTintImage.css({ left: f.tintpos + "px" });
        f.zoomTintImage.css({ top: f.tintposy + "px" });
      }
    },
    ...
    swapAction: function (h, g) {
      ...
      if (f.options.zoomType == "lens") {
        f.zoomLens.css({ backgroundImage: "url('" + g + "')" });
      }
      if (f.options.zoomType == "window") {
        f.zoomWindow.css({ backgroundImage: "url('" + g + "')" });
      }
      if (f.options.zoomType == "inner") {
        f.zoomWindow.css({ backgroundImage: "url('" + g + "')" });
      }
      ...
      if (f.options.imageCrossfade) {
        ...
        if (f.options.tint && f.options.zoomType != "inner") {
          ...
          f.zoomTint.css({ height: f.$elem.height() });
          f.zoomTint.css({ width: f.$elem.width() });
        }
        f.zoomContainer.css("height", f.$elem.height());
        f.zoomContainer.css("width", f.$elem.width());
        if (f.options.zoomType == "inner") {
          if (!f.options.constrainType) {
            f.zoomWrap.parent().css("height", f.$elem.height());
            f.zoomWrap.parent().css("width", f.$elem.width());
            f.zoomWindow.css("height", f.$elem.height());
            f.zoomWindow.css("width", f.$elem.width());
          }
        }
        if (f.options.imageCrossfade) {
        ...
        if (f.options.tint) {
          ...
          f.zoomTintImage.css({ height: f.$elem.height() });
          f.zoomTint.css({ height: f.$elem.height() });
        }
        f.zoomContainer.css("height", f.$elem.height());
        f.zoomContainer.css("width", f.$elem.width());
      ...
      if (f.options.constrainType) {
        if (f.options.constrainType == "height") {
          f.zoomContainer.css("height", f.options.constrainSize);
          f.zoomContainer.css("width", "auto");
          if (f.options.imageCrossfade) {
          ...
          if (f.options.zoomType == "inner") {
            ...
            f.zoomWindow.css("height", f.options.constrainSize);
            f.zoomWindow.css("width", f.constwidth);
          }
          if (f.options.tint) {
        ...
        if (f.options.constrainType == "width") {
          f.zoomContainer.css("height", "auto");
          f.zoomContainer.css("width", f.options.constrainSize);
          if (f.options.imageCrossfade) {
          ...
          if (f.options.zoomType == "inner") {
            ...
            f.zoomWindow.css("height", f.constheight);
            f.zoomWindow.css("width", f.options.constrainSize);
          }
          if (f.options.tint) {
    ...
    getGalleryList: function () {
      ...
      if (f.options.gallery) {
        d("#" + f.options.gallery + " a").each(function () {
          var g = "";
          if (d(this).data("zoom-image")) {
            g = d(this).data("zoom-image");
          } else {
            if (d(this).data("image")) {
              g = d(this).data("image");
            }
          }
          if (g == f.zoomImage) {
    ...
    changeZoomLevel: function (g) {
      ...
      maxheightnewvalue =
        f.largeHeight /
        ((f.options.zoomWindowHeight / f.nzHeight) * f.nzHeight);
      maxwidthtnewvalue =
        f.largeWidth / ((f.options.zoomWindowWidth / f.nzWidth) * f.nzWidth);
      if (f.options.zoomType != "inner") {
  ...
  d.fn.elevateZoom = function (f) {
    return this.each(function () {
      var g = Object.create(b);
      g.init(f, this);
      d.data(this, "elevateZoom", g);
    });
  };
